...
export default function ImageModify({ data, imageSizes }: modifyTypes) {
  ...
  const [imageName, setImageName] = useState<string | null>(localStorage.getItem('fileName'));
  ...
  img.onload = () => {
    if (load) {
      imageSizes({ ...data, count: 0, imageWidth: img.width, imageHeight: img.height });
  ...
  useEffect(() => {
    img.onerror = () => {
      // If does not exist redirect default root
      router.push("/");
    }
  }, [])
  ...
  return (
    <div>
      <div id="uploaded-image" >
        ...
        <div ref={loaderImage}>
          <Image src={'/images/'+(imageName ? imageName.substring(1, imageName.length - 1) : '')} id="newimg" width={imageWH[0]} height={imageWH[1]} alt="Modified" className={mirror}/>
        </div>
      </div>
    </div>
  )
...
function imageResize(img: any, setImgWH: Function) {
  ...
  if (img.width > 1000 && img.height > 600) { //If the width and height of the image are large in the canvas dimensions
  ...
  } else if (img.width > 1000 || img.height > 600) { //If the width or height of the image in the canvas dimensions is large
    if (img.width > img.height) {
      ratio = ((img.width === 1000 ? img.width + img.height : img.width) - 1000) / img.width;
      imgHeight = img.height - (img.height * ratio);
